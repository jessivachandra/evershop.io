...
describe('Shopping With Login Story', () => {
    it('Shopping', () => {
        ...
        cy.get("input[placeholder='Email']").type("jessi1@gmail.com");
        cy.get("input[placeholder='Password']").type("Jessi123*");
        cy.get("button[type='submit']").click();
        cy.wait(1500)

        cy.fixture('shoppingwithlogin').then((data) => {
            data.forEach((userdata) => {
                ...
                cy.get('input[placeholder="Full name"]').click().type(userdata.fullname);
                cy.get('input[placeholder="Telephone"]').click().type(userdata.phone);
                cy.get('input[placeholder="Address"]').click().type(userdata.address);
                cy.get('input[placeholder="City"]').click().type(userdata.city);
                cy.get('select[id="address\\[country\\]"]').select(userdata.country);
                cy.get('select[id="address\\[province\\]"]').select(userdata.province);
                cy.get('input[placeholder="Postcode"]').click().type(userdata.postcode);
                cy.wait(3500)
